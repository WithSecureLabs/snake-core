...
from celery.app.control import \
    Inspect  # pylint: disable=import-error, no-name-in-module
from tornado import ioloop, web
from tornado.log import enable_pretty_logging
from tornado.options import options

from snake.config import config_parser, constants, snake_config
...
def main(
    config_file=None, debug=False, pidfile=None
    ...
    application = web.Application(
        routes, debug=debug, autoreload=debug, default_handler_class=DefaultHandler
    )
    application.listen(
        snake_config["port"],
        address=snake_config["address"],
...
def __main__():
    ...
    parser.add_argument(
        "--pidfile", dest="pidfile", default=None, help="path to PID file"
    )
