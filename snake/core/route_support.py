...
from snake import db, enums, error, schema, utils
...
async def queue_command(data):
    ...
    if document:
        if "status" in document and document["status"] == enums.Status.RUNNING:
            ...
            await db.async_command_collection.replace(
                data["sha256_digest"],
                data["scale"],
                data["command"],
                data["args"],
    ...
    if data["asynchronous"] is True:
        celery.execute_command.apply_async(
            args=[data],
        ...
        if not task.successful():
            ...
            _new_output_id = await db.async_command_output_collection.put(
                document["command"], b"{'error': 'worker failed please check log'}"
            )
            document["_output_id"] = _new_output_id
            document["status"] = enums.Status.FAILED
            await db.async_command_collection.update(
    ...
    return await db.async_command_collection.select(
        data["sha256_digest"], data["scale"], data["command"], data["args"]
    )
...
async def unzip_file_python(
    file_path, file_name, output_dir, protected=False, password=None
):
    ...
    if protected:
        if password:
            try:
                new_path = zip_file.extract(
                    file_name, output_dir, bytes(password, "utf-8")
                )
            ...
            for passwd in snake_config["zip_passwords"]:
                try:
                    new_path = zip_file.extract(
                        file_name, output_dir, bytes(passwd, "utf-8")
                    )
                except RuntimeError as err:
                    if "Bad password" not in str(err):
                        raise
...
async def unzip_file(file_path, password=None):
    ...
    if outsource:
        new_path = await unzip_file_unix(
            file_path, i.filename, working_dir, protected, password
        )
    else:
        new_path = await unzip_file_python(
            file_path, i.filename, working_dir, protected, password
        )
    if not new_path:
