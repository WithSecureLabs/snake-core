...
class UploadFileHandler(snake_handler.StreamHandler):
    ...
    async def post(self, data):
        ...
        if document:
            ...
            self.write_warning(
                "upload/file - file already exists for given sha256 digest",
                409,
                {"sample": document},
            )
            self.finish()
        ...
        document = await route_support.store_file(
            sha256_digest, f_path, enums.FileType.FILE, data
        )
        document = schema.FileSchema().dump(schema.FileSchema().load(document))
...
class UploadFilesHandler(snake_handler.StreamHandler):
    ...
    async def post(self):
        ...
        try:
            ...
            self.write_warning(
                "upload/files - must be content type application/json", 422, data
            )
            self.finish()
            return
        ...
        for k, v in data.items():
            ...
            if document:
                ...
                documents += [
                    await route_support.store_file(
                        sha256_digest, f_path, enums.FileType.FILE, v
                    )
                ]
...
class UploadMemoryHandler(snake_handler.StreamHandler):
    ...
    async def post(self, data):
        ...
        if document:
            document = schema.FileSchema().dump(schema.FileSchema().load(document))
            self.write_warning(
                "upload/memory - memory already exists for given sha256 digest",
                409,
                {"sample": document},
            )
            self.finish()
        ...
        # Save the file and add it to the database
        document = await route_support.store_file(
            sha256_digest, f_path, enums.FileType.MEMORY, data
        )
        document = schema.FileSchema().dump(schema.FileSchema().load(document))
        self.jsonify({"sample": document})
        self.finish()
