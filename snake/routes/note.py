...
from snake import db, fields, schema
...
class NoteHandler(snake_handler.SnakeHandler):
    ...
    async def get(self, sha256_digest):
        ...
        if not document:
            self.write_warning(
                "note - no sample for given sha256 digest", 404, sha256_digest
            )
            self.finish()
            return
...
class NotePostHandler(snake_handler.SnakeHandler):
    ...
    async def post(self, data):
        ...
        if document:
            document = schema.NoteSchema().dump(schema.NoteSchema().load(document))
            self.write_warning(
                "note - note already exists for given data", 409, {"note": document}
            )
            self.finish()
            return
...
NoteRoute = (
    r"/note/(?P<sha256_digest>[a-zA-Z0-9]+)?",
    NoteHandler,
